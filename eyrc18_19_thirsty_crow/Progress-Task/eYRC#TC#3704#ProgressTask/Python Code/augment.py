...
* Theme: Thirsty Crow
* Functions: None
* Global Variables: pitcher_level,pebble pebble_model,pitcher_model_low ,pitcher_model_high
'''
...
pebble_model =None
pitcher_model_low  = None
pitcher_model_high = None
...
def init_gl():
    ...
    global pebble_model
    global pitcher_model_low
    global pitcher_model_high
    ...
    pitcher_model_low= OBJ('pitcher_low.obj', swapyz=True)
    pitcher_model_high = OBJ('pitcher_high.obj', swapyz=True)
...
def drawGLScene():
    ...
    if ret == True:
        ...
        if (_ == True):
            ...
            for i in ar_list:
                # set pitcher
                if i[0] == 1 and pitcher_level =="LOW":
                    overlay(frame, ar_list, i[0], pitcher_model_low)
                if i[0] == 1 and pitcher_level =="HIGH":
                    overlay(frame, ar_list, i[0], pitcher_model_high)
                # set PEBBLE
                if i[0] == 0 and pebble =="YES":
...
def overlay(img, ar_list, ar_id, model):
    ...
    view_matrix = np.array(
        [[rmtx[0][0], rmtx[0][1], rmtx[0][2], tvec[0][0][0]], [rmtx[1][0], rmtx[1][1], rmtx[1][2], tvec[0][0][1]],
         [rmtx[2][0], rmtx[2][1], rmtx[2][2], tvec[0][0][2]], [0.0, 0.0, 0.0, 1.0]])

...
* Theme: Thirsty Crow
* Functions: None
* Global Variables: pitcher_level,pebble pebble_model,pitcher_model_zero ,pitcher_model_high
'''
...
crow_model = None
pebble_model =None
pitcher_model_zero  = None
pitcher_model_one = None
pitcher_model_two = None
pitcher_model_three = None
...
def init_gl():
    ...
    global crow_model
    global pebble_model
    global pitcher_model_zero
    global pitcher_model_one
    global pitcher_model_two
    global pitcher_model_three
    ...
    pebble_model = OBJ('pebble.obj', swapyz=True)
    pitcher_model_zero= OBJ('pitcher_zero.obj', swapyz=True)
    pitcher_model_one = OBJ('pitcher_one.obj', swapyz=True)
    pitcher_model_two = OBJ('pitcher_two.obj', swapyz=True)
    pitcher_model_three= OBJ('pitcher_three.obj', swapyz=True)
    crow_model =  OBJ('Crow.obj', swapyz=True)
    ...
    if ret == True:
        ...
        if (_ == True):
            ...
            for i in ar_list:
                ...
                if i[0] == 0 :
                    if id_0 == 0:
                        overlay(frame, ar_list, i[0], pitcher_model_zero)
                    if id_0 == 1:
                        overlay(frame, ar_list, i[0], pitcher_model_one)
                    if id_0 == 2:
                        overlay(frame, ar_list, i[0], pitcher_model_two)
                    if id_0 == 3:
                        overlay(frame, ar_list, i[0], pitcher_model_three)
                if i[0] == 2 :
                    if id_2 ==1 :

                        overlay(frame, ar_list, i[0], pebble_model)
                if i[0] == 4 :
                    if id_4 ==1:
                        overlay(frame, ar_list, i[0], pebble_model)
                if i[0] == 6 :
                    if id_6 ==1:
                        ...
                        overlay(frame, ar_list, i[0], pebble_model)
                if i[0] == 10 :
                    overlay(frame, ar_list, i[0], crow_model)
...
def detect_markers(img):
    ...
    img = aruco.drawDetectedMarkers(img, corners, ids, borderColor=(225, 0, 255))
...
def overlay(img, ar_list, ar_id, model):
    for x in ar_list:
        if ar_id == x[0]:
    ...
    # tvec[0][0][2] = tvec[0][0][2] / (500)
    ##        print('(',tvec[0][0][0], tvec[0][0][1],tvec[0][0][2],')')

    rmtx = cv2.Rodrigues(rvec)[0]

    view_matrix = np.array([[rmtx[0][0], rmtx[0][1], rmtx[0][2], tvec[0][0][0]*1.5],
                            [rmtx[1][0], rmtx[1][1], rmtx[1][2],tvec[0][0][1]*1.5],
